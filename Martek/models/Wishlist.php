...
//end METHOD -   set the page heading
    ...
    {//set the panel 1 heading
        if ($this->loggedin)
            {
            $this->panelHead_1 = '<h3>My Wishlist</h3>';
            }
        else
            {
...
//end METHOD - //set the panel 1 heading
 ...
 //userID   
                $Username=$this->user->getUserID();
                    ...
                    $sql='SELECT * FROM products as p inner join wishlist as w where w.UserID='.$row["UserID"].' AND w.ProductID=p.ProductID';
            ...
            if ((@$rs = $this->db->query($sql)) && ($rs->num_rows))
                ...
                if (!$rs->num_rows)
                    ...
                    while ($row = $rs->fetch_assoc())
                        ...
                        $this->panelContent_1.='<form method="post" action="index.php?pageID=wishlist">';
                        ...
                        $this->panelContent_1 .='<p><button type="submit" id="addtocart" name="removefromwishlist" value="'.$row["ProductID"].'">Remove From Wishlist</button></p>';
                ...
                if (isset($_POST['removefromwishlist']))
                    ...
                    $SQL='SELECT UserID FROM users WHERE username="'.$Username.'"';
                        ...
                        $DELETE='DELETE FROM wishlist WHERE ProductID='.$_POST['removefromwishlist'].'';
                        echo $DELETE;
                         $rs=$this->db->query($DELETE); 
        //check the insert query worked
                        if ($rs)
                        {
                            echo '<h4>Item Removed From Wishlist</h4>';
                            header("Location: index.php?pageID=wishlist");
                         return TRUE;
                        }
                        else
                        {
