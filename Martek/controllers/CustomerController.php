        ...
        if (isset($this->getArray['pageID'])) { //check if a page id is contained in the URL
            switch ($this->getArray['pageID']) {
                case "home":
                    ...
                    //update the view
                    include_once 'views/view_navbar_Home_LoggedIn.php';  //load the view
                    break;
                case "viewProducts":             
                    //get the model
                    $ViewProducts = new ViewProducts($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$ViewProducts,$navigation);
                    //get the content from the model - put into the $data array for the view:
                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS                    
                    //get the content from the model - put into the $data array for the view:
                    $data['pageTitle'] = $ViewProducts->getPageTitle();
                    $data['pageHeading'] = $ViewProducts->getPageHeading();
                    $data['panelHeadRHS'] = $ViewProducts->getPanelHead_3(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $ViewProducts->getPanelHead_1(); // A string containing the LHS panel heading/title
                    $data['panelHeadMID'] = $ViewProducts->getPanelHead_2();
                    $data['stringLHS'] = $ViewProducts->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringMID'] = $ViewProducts->getPanelContent_2();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $ViewProducts->getPanelContent_3();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                    include_once 'views/view_navbar_1_panel_Down.php'; //load the view
                    break;
           
                case "account":
                    //create objects to generate view content
                    $Account = new Account($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                ...
                case "wishlist":
                    //create objects to generate view content
                    $Wishlist = new Wishlist($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$Wishlist,$navigation);

                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS
                    //get the content from the page content model  - put into the $data array for the view:
                    $data['pageTitle'] = $Wishlist->getPageTitle();
                    $data['pageHeading'] = $Wishlist->getPageHeading();
                    $data['panelHeadRHS'] = $Wishlist->getPanelHead_2(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $Wishlist->getPanelHead_1(); // A string containing the LHS panel heading/title 
                    $data['stringLHS'] = $Wishlist->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $Wishlist->getPanelContent_2();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                    include_once 'views/view_navbar_1_panel_Down.php'; //load the view
                    break;
                ...
                case "cart":
                    //create objects to generate view content
                    $cart = new Cart($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$cart,$navigation);

                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS
                    //get the content from the page content model  - put into the $data array for the view:
                    $data['pageTitle'] = $cart->getPageTitle();
                    $data['pageHeading'] = $cart->getPageHeading();
                    $data['panelHeadRHS'] = $cart->getPanelHead_2(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $cart->getPanelHead_1(); // A string containing the LHS panel heading/title 
                    $data['stringLHS'] = $cart->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $cart->getPanelContent_2();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                    include_once 'views/view_navbar_2_panel_Down.php'; //load the view
                    break;
                
                case "orders":
                    //create objects to generate view content
                    $Orders = new Orders($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$Orders,$navigation);

                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS
                    //get the content from the page content model  - put into the $data array for the view:
                    $data['pageTitle'] = $Orders->getPageTitle();
                    $data['pageHeading'] = $Orders->getPageHeading();
                    $data['panelHeadRHS'] = $Orders->getPanelHead_2(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $Orders->getPanelHead_1(); // A string containing the LHS panel heading/title 
                    $data['stringLHS'] = $Orders->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $Orders->getPanelContent_2();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                    include_once 'views/view_navbar_1_panel_Down.php'; //load the view
                    break;
                ...
                case "logout":
                    ...
                    //update the view
                    include_once 'views/view_navbar_Home.php'; //load the view                  
