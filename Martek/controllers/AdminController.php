 ...
 * This is the controller for the LECTURER user type
 *
    ...
    public function updateView() { //update the VIEW based on the users page selection
        if (isset($this->getArray['pageID'])) { //check if a page id is contained in the URL
            switch ($this->getArray['pageID']) {
                ...
                case "customerAdmin":
                    //get the model
                    $CustomerAdmin = new CustomerAdmin($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$CustomerAdmin,$navigation);
                    //get the content from the model - put into the $data array for the view:
                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS                    
                    //get the content from the model - put into the $data array for the view:
                    $data['pageTitle'] = $CustomerAdmin->getPageTitle();
                    $data['pageHeading'] = $CustomerAdmin->getPageHeading();
                    $data['panelHeadRHS'] = $CustomerAdmin->getPanelHead_3(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $CustomerAdmin->getPanelHead_1(); // A string containing the LHS panel heading/title
                    $data['panelHeadMID'] = $CustomerAdmin->getPanelHead_2();
                    $data['stringLHS'] = $CustomerAdmin->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringMID'] = $CustomerAdmin->getPanelContent_2();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $CustomerAdmin->getPanelContent_3();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                ...
                case "viewOrders":
                    //get the model
                    $AViewOrders = new AdminViewOrders($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$AViewOrders,$navigation);

                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS
                    //get the content from the model - put into the $data array for the view:
                    $data['pageTitle'] = $AViewOrders->getPageTitle();
                    $data['pageHeading'] = $AViewOrders->getPageHeading();
                    $data['panelHeadRHS'] = $AViewOrders->getPanelHead_2(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $AViewOrders->getPanelHead_1(); // A string containing the LHS panel heading/title
                    $data['stringLHS'] = $AViewOrders->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $AViewOrders->getPanelContent_2();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                    include_once 'views/view_navbar_1_panel_Down.php'; //load the view
                    break;            
                case "manageProducts":
                    //create objects to generate view content
                    $ManageProducts = new ManageProducts($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user);
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$ManageProducts,$navigation);

                    //get the content from the navigation model - put into the $data array for the view:
                    $data['menuNav'] = $navigation->getMenuNav();       // an array of menu items and associated URLS
                    //get the content from the page content model  - put into the $data array for the view:
                    $data['pageTitle'] = $ManageProducts->getPageTitle();
                    $data['pageHeading'] = $ManageProducts->getPageHeading();
                    $data['panelHeadRHS'] = $ManageProducts->getPanelHead_3(); // A string containing the RHS panel heading/title
                    $data['panelHeadLHS'] = $ManageProducts->getPanelHead_1(); // A string containing the LHS panel heading/title
                    $data['panelHeadMID'] = $ManageProducts->getPanelHead_2();
                    $data['stringLHS'] = $ManageProducts->getPanelContent_1();     // A string intended of the Left Hand Side of the page
                    $data['stringMID'] = $ManageProducts->getPanelContent_2();     // A string intended of the Left Hand Side of the page
                    $data['stringRHS'] = $ManageProducts->getPanelContent_3();     // A string intended of the Right Hand Side of the page
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purposes
                    //update the view
                    include_once 'views/view_navbar_2_Panel_Wide.php'; //load the view
                    break;
                case "account":
                    //create objects to generate view content
